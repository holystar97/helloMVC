 ...
 */
/**
 * servlet annotations를 사용하여, servlet name은 파일 이름명이자 클래스명 인 DoLogin 을 쓰고, servlet의 url-pattern 은 
 * 아래와 같이 webservlet annotation을 사용하여 /doLogin 과 같이 경로명을 만들어준다. 
 * */
...
public class DoLogin extends HttpServlet {
    ...
    /**
     * business logic 처리 부분 
     * */
    
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// post방식으로 전달받은 name parameter를 가져와서 처리하는 부분
		
		String id = request.getParameter("id");
		String password = request.getParameter("password");
		
		// customerSerivce 객체 생성 - 싱글톤 패턴/ 프로그램 종료될 때 까지 계속 사용 
		CustomerService service = (CustomerService) CustomerService.getInstance();
		
		// customer login 의 결과 값에 따라 페이지를 이동시키고자 한다. 
